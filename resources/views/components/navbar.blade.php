<nav class="bg-white border-b border-border fixed top-0 w-full z-50">
  <div class="max-w-[120rem] mx-auto px-6 h-20 flex items-center justify-between gap-4">
    ...
    <div class="flex items-center gap-2 shrink-0">
      ...
      <div class="hidden lg:flex items-center gap-1 mr-2">
        ...
        @auth
          <a href="{{ route('favorites.index') }}"
             class="text-sm font-semibold px-4 py-2.5 rounded-full transition {{ request()->routeIs('favorites.index') ? 'text-primary bg-primary/10' : 'text-ink hover:bg-surface' }}">
            Favoris
          </a>
        @endauth
      </div>

      {{-- Bouton Création (si autorisé) --}}
      @can('create', App\Models\Logement::class)
        <a href="{{ route('logements.create') }}" 
           class="hidden sm:block text-sm font-semibold hover:bg-surface px-4 py-2.5 rounded-full transition border border-transparent hover:border-border">
          Publier un logement
        </a>
      @endcan

      {{-- Icône Globe --}}
      <button class="p-2.5 rounded-full hover:bg-surface transition hidden sm:block">
      ...
      {{-- Menu Utilisateur (Pill) --}}
      <button class="flex items-center gap-3 border border-border rounded-full pl-3 pr-1.5 py-1.5 hover:shadow-md transition-shadow bg-white">
        <svg class="w-5 h-5 text-ink" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"/>
        </svg>
        <div class="w-8 h-8 bg-muted rounded-full flex items-center justify-center overflow-hidden">
          @auth
            {{-- Initiale de l'utilisateur ou icône par défaut --}}
